...
export default defineConfig({
  server: {
    ...
    proxy: {
      ...
      // API 서버 (쿠버네티스 NodePort 30084) — 공연 목록·인증·대기열·좌석·예매·SSE 등 주요 비즈니스 로직
      // 찬규님 파트가 K8s로 이전되면서 단독 VM(192.168.0.190:2000) 대신 클러스터 마스터 노드 IP + NodePort로 접근
      '/events':       { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/auth':         { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/queue':        { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/seats':        { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/event':        { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/admin':        { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/sse':          { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/reservations': { target: 'http://192.168.0.192:30084', changeOrigin: true },
      // 위시리스트/멤버십은 A파트에 이미 완전한 API(add/remove/subscribe/cancel)가 있어
      // B파트(건아)의 중복 조회 API 대신 A파트로 연결 — B에는 add/remove가 없어 하트 토글이 깨져있었음
      '/membership': { target: 'http://192.168.0.192:30084', changeOrigin: true },
      '/wishlist':   { target: 'http://192.168.0.192:30084', changeOrigin: true },
