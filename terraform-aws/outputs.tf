...
output "nat_eip" {
  description = "NAT Instance 공인 IP — 프라이빗 서브넷의 모든 아웃바운드가 이 주소로 나간다"
  value       = aws_eip.nat.public_ip
}

output "dcloud_secret_arn" {
  description = "D-Cloud DB 접속 정보 Secrets Manager ARN"
  value       = aws_secretsmanager_secret.dcloud_db.arn
}
