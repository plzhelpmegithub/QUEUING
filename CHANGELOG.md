...
### 🔄 변경 및 수정 사항
- **[src/services/seatService.js]**: 공연 취소·삭제 시 Redis 좌석 키를 한 번의 대량 `DEL`로 제거하던 로직을 500개 단위 `UNLINK` 순차 처리로 변경. 기존 반환값과 삭제 API 흐름은 유지한다.

### 🛠 트러블슈팅 (Troubleshooting)
- **증상(Issue):** 좌석이 수천 개씩 생성된 공연이 다수 존재할 때 공연 한 건 삭제도 오래 걸리고, 앞단 게이트웨이가 먼저 `504 Gateway Timeout`을 반환할 수 있음.
- **원인(Cause):** `cleanupEventSeats()`가 공연별 좌석 키 수천 개를 단일 `DEL` 명령으로 전달하여 Redis 이벤트 루프에서 동기 메모리 해제 부담이 한 번에 집중됨.
- **해결(Solution):** 좌석 키를 500개씩 나누고 `UNLINK`로 Redis 키 공간에서 우선 분리한 뒤 실제 메모리 회수를 백그라운드에 맡겨, 삭제 중 다른 대기열·좌석 요청이 받는 순간 지연을 줄임.
...
### 🔄 변경 및 수정 사항
- **[src/routes/cancelQueueRoutes.js]**: `GET /cancel-queue/history/mine`이 이력 테이블만 조회하도록 분리. 이력 표시와 무관한 `events` 조인 및 요청 시점의 DDL 실행을 제거해, 공연 메타데이터 조회 실패가 양도 이력 전체를 500으로 막지 않도록 보완.

### 🛠 트러블슈팅 (Troubleshooting)
- **증상(Issue):** 마이페이지 취소/환불내역에서 `GET /cancel-queue/history/mine`이 500으로 실패해 DB에 저장된 양도 이력이 표시되지 않음.
- **원인(Cause):** 사용자별 양도 이력 조회가 표시 보조 정보인 `events` 테이블과 런타임 테이블 생성에 함께 의존하고 있었음.
- **해결(Solution):** 이력 조회를 `cancel_queue_history` 단일 테이블로 한정. 공연명은 프론트엔드의 이미 로드된 공연 목록으로 보완한다.
