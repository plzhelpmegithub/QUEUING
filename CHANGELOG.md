...
### 🔄 변경 및 수정 사항
- **[src/pages/admin.js]**: 관리 기능(공연 생성, 포스터 공연 생성, 개별/일괄 삭제, 오픈 시간, 마감 시간, 즉시 마감, Redis 초기화, DB→Redis 복구)에 `addNotification` 알림 추가. 기존에는 시뮬레이션만 알림이 발생했으나, 이제 모든 관리 액션에서 알림을 수신 가능.
- **[src/pages/admin.js]**: 공연 삭제 시 `warnings` 배열을 확인하여 부분 실패 여부를 토스트로 표시하도록 개선. 일괄 삭제에서도 동일하게 `warnings` 기준으로 확인 필요 항목을 판별.
- **[src/pages/admin.js]**: 워커 토글을 "Auto Recovery" 전용으로 변경. SyncRetry Worker 제어 제거. 라벨을 "Auto Recovery"로 변경하고 부가 설명에 "(5분 주기 DB→Redis 복구)" 추가.
- **[src/pages/admin.js]**: 사이드바 하단의 "?" 커맨드 팔레트 단축키 표시 제거.
...
### 🔄 변경 및 수정 사항
- **[src/pages/admin.js]**: 어드민 개요 섹션에 "백그라운드 워커" 토글 UI 추가. Redis Auto Recovery(5분 주기)와 SyncRetry Worker(30초 주기)의 실행 상태를 표시하고, 버튼 클릭으로 일시 정지/재개 가능. 공연 삭제 및 시뮬레이션 데이터 삭제 전에 정지 권장 안내 포함. `GET /admin/workers/status`, `POST /admin/workers/pause`, `POST /admin/workers/resume` API 연동.
...
### 🔄 변경 및 수정 사항
- **[src/pages/admin.js]**: B파트 링크 발급 완료(`link_requested` 단계) 후 "데이터 삭제" 버튼에 빨간색 펄스 리플 효과(`btn-attention` 클래스) 추가. 데이터 삭제를 잊지 않도록 시각적으로 강조됨.
- **[src/pages/admin.js]**: 통합 시뮬레이션 패널에 "테스트 가이드" 토글 버튼 추가. 본 티켓팅 시뮬과 취소표 링크 테스트 두 탭으로 구분하여 각각의 테스트 절차를 안내 텍스트로 표시.
- **[src/styles/global.css]**: `btn-attention` 클래스 — 빨간색 box-shadow 펄스 애니메이션 추가.

## [2026-09-23 18:10] 업데이트 로그

### 🔄 변경 및 수정 사항
- **[src/pages/admin.js]**: 통합 시뮬레이션 패널의 모든 단계별 버튼 핸들러(초기화·본 티켓팅 대기열·좌석 선점·대기열 드레인·매진 및 전환·티켓팅 마감·취소표 생성·멤버십 더미 삭제·B파트 링크 발급)에서, API 호출 성공/실패에 관계없이 `finally`에서 항상 `/status`를 재조회하여 버튼 활성화 상태를 갱신하도록 변경.

### 🛠 트러블슈팅 (Troubleshooting)
- **증상(Issue):** 시뮬레이션 단계 완료 후 다음 단계 버튼이 비활성화 상태로 유지됨 (예: 매진 전환 후 "티켓팅 마감" 비활성, 취소표 생성 후 "B파트 링크 발급" 비활성). "상태 새로고침"을 눌러야만 활성화됨.
- **원인(Cause):** 각 단계 API 호출 후 `/status` 재조회를 `.then` 체인 안에서만 실행했기 때문에, API가 에러 응답(409 등)을 반환하면 `simFetch`가 throw하여 `.catch`로 빠지고 `/status` 재조회가 건너뛰어짐. `finally`에서는 이전 `currentStage` 값으로 `updateButtons()`를 실행하여 다음 단계 버튼이 비활성 상태로 유지됨.
- **해결(Solution):** 모든 핸들러에서 `/status` 재조회를 `.then` 체인에서 제거하고 `finally` 블록으로 이동. 성공/실패와 무관하게 항상 서버의 최신 stage를 조회하여 버튼 상태를 올바르게 갱신.
