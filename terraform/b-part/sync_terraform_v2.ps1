# QUEUING B-part Terraform - FULL SYNC (v2)
# Fixes: EKS iam.tf now references existing pre-provisioned
# IAM roles (queuing-eks-cluster-role, queuing-eks-node-role)
# instead of trying to create new ones (account has no
# iam:CreateRole permission).
# Run this FROM INSIDE the terraform folder.
# Usage: .\sync_terraform_v2.ps1
...
@'
# This AWS account has iam:CreateRole disabled for students - an admin
# pre-provisioned the roles this project needs. Reference them instead
# of trying to create new ones.
data "aws_iam_role" "cluster" {
  name = "queuing-eks-cluster-role"
}

data "aws_iam_role" "node" {
  name = "queuing-eks-node-role"
}
...
Write-Host "Done. All terraform files synced (IAM role fix applied)."
