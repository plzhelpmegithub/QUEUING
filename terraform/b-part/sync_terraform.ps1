# QUEUING B-part Terraform - FULL SYNC
# Regenerates ALL .tf files (network, sqs, eks, db modules + root)
# to the current correct/consistent state. Safe to re-run anytime -
# always use this one going forward instead of the per-module scripts.
# Run this FROM INSIDE the terraform folder.
# Usage: .\sync_terraform.ps1
...
Write-Host "Done. All terraform files synced to latest version."
Write-Host "Modules: network, sqs, eks, db"
