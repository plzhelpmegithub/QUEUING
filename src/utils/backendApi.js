...
export async function fetchRealSeats() {
  const data = await getJson('/seats');
...
export async function holdSeatApi(userId, seatId, token) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
...
export async function releaseSeatApi(userId, seatId) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
...
export async function confirmSeatApi(userId, seatId) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
