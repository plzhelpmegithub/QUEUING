// 마이페이지 전용 렌더링 헬퍼 — 예매내역·환불내역·관심공연 카드 HTML을 생성하는 함수 모음.
...
function daysUntilShow(b, c) {
  const dateStr = b.session?.date || c.dateStart.slice(0, 10);
...
function openRefundConfirm(b, c) {
  const days = daysUntilShow(b, c);
  const rate = calcCancelFeeRate(days);
  const fee = Math.round(b.price * rate);
  const refundAmount = Math.max(0, b.price - fee);
  const dateLabel = (b.session?.date || c.dateStart.slice(0, 10)).replaceAll('-', '.');

  openModal({
    title: '예매를 취소하시겠습니까?',
    bodyHtml: `
      <p style="margin-bottom:14px;"><strong>${c.artist} · ${c.title}</strong> 티켓을 환불하시겠습니까?</p>
      <div class="kv-row"><span>공연일</span><b>${dateLabel}${b.session?.time ? ' ' + b.session.time : ''}</b></div>
      <div class="kv-row"><span>좌석</span><b>${b.seat.gradeName} ${b.zone?.label || b.seat.section} ${b.seat.row}열 ${b.seat.seatNum}번</b></div>
...
export const myPage = {
  render(container, params) {
    ...
    function renderRefunds() {
      const list = bookings.filter((b) => b.status === 'refund_pending' || b.status === 'refunded');
      content.innerHTML = `
        <div class="mypage-section-title" style="margin-top:0;">취소/환불내역</div>
        ${list.length ? list.map((b) => refundRowHtml(b)).join('') : emptyRow('취소 및 환불 내역이 없습니다.')}
      `;
    }
    ...
    function bookingRowHtml(b) {
      const c = getConcert(b.concertId);
      if (!c) return '';
      const dateLabel = b.session?.date ? b.session.date.replaceAll('-', '.') : formatDate(c.dateStart);
      return `
        <div class="ticket-row" style="align-items:flex-start;">
          <div>
            <div class="ticket-row__concert">${c.artist} · ${c.title} ${b.source === 'cancel' ? '<span class="badge badge-red-light">취소표</span>' : ''}</div>
            <div class="ticket-row__meta">${dateLabel}${b.session?.time ? ' ' + b.session.time : ''} · ${b.seat.gradeName} ${b.zone?.label || b.seat.section} ${b.seat.row}열 ${b.seat.seatNum}번</div>
          </div>
          <div style="text-align:right;">
    ...
    function wireBookingRows(scopeEl) {
      ...
      scopeEl.querySelectorAll('[data-refund]').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          const concert = booking && getConcert(booking.concertId);
          if (booking && concert) openRefundConfirm(booking, concert);
        });
      });
    }
