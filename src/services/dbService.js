...
async function initTable({ includeLastSimulation = false } = {}) {
  await pool.query(`
  ...
  if (includeLastSimulation) {
    ...
    `);
    console.log('[MariaDB] 공통 테이블 + Last 로컬 시뮬레이션 테이블 준비 완료');
  } else {
    console.log('[MariaDB] 공통 테이블 준비 완료 (Last 로컬 시뮬레이션 테이블 제외)');
  }
}
