...
SQL로 묶어서 한 방에 단순하게 처리한다.

비즈니스 규칙(2026-09-11 확정): 재판매 순번배정은 멤버십 회원에게만 적용한다.
비멤버십 회원은 대기열에 있어도 후보에서 완전 제외한다 (우선순위 문제가 아니라
자격 조건). 지금은 waiting_queue.membership_at_join(대기열 진입 시점에 멤버십인
여부)으로 판단한다 — 진입 이후 멤버십이 바뀐 사람을 정확히 반영하려면 memberships
테이블을 조인해서 "현재" 상태로 판단해야 하는데, 재판매가 마감 후 1주일 이내에
끝나는 짧은 기간이라 우선은 이 값으로 충분하다고 보고 진행. 나중에 문제되면
memberships 테이블 조인으로 교체.

[2026-09-19 patch] event_id만으로 후보를 조회해서, 같은 event_id에 여러 회차
(session_date/session_time)가 있는 경우 다른 회차 대기자가 후보로 뽑히는
버그가 있었다. 취소 이벤트가 특정 회차 것이면 반드시 그 회차의 대기자만
후보로 조회해야 한다 — 안 그러면 이후 GenerateSignedLink가 cancel_allocations에
빈/잘못된 회차로 저장하고, A파트 상태 조회 API가 본 대기열 참여 이력을
엉뚱한 회차 기준으로 찾다가 main_queue_required를 잘못 반환한다.
...
# 엇갈려있어서 utf8mb4_general_ci vs utf8mb4_unicode_ci 그냥 비교하면
# "Illegal mix of collations" 에러가 난다. 비교하는 모든 컬럼 식에 COLLATE를
# 명시해서 강제로 맞춘다. (근본적으로는 언젠가 전체 테이블 collation을
# 통일하는 게 맞지만, 지금은 쿼리 레벨에서 우회)
#
# [2026-09-19 patch] session_date/session_time 필터 추가 — 같은 event_id라도
# 회차가 다르면 서로 다른 대기열로 취급해야 한다.
_QUERY = f"""
    SELECT wq.user_id, wq.queue_index
    FROM waiting_queue wq
    WHERE wq.event_id = %s
      AND wq.session_date = %s
      AND wq.session_time = %s
...
if __name__ == "__main__":
    # 로컬 테스트: python get_next_user.py evt-9 2026-11-14 18:00
    target_event_id = sys.argv[1] if len(sys.argv) > 1 else "evt-9"
    target_session_date = sys.argv[2] if len(sys.argv) > 2 else "2026-11-14"
    target_session_time = sys.argv[3] if len(sys.argv) > 3 else "18:00"
    result = handler({
        "event_id": target_event_id,
        "session_date": target_session_date,
        "session_time": target_session_time,
    })
    print(result)
